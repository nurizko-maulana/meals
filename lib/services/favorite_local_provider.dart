...
class FavoriteLocalProvider {
  ...
  Future<List<Meal>> getFavoriteMeals() async {
    ...
    List<Meal> meals = result!.isEmpty
        ? []
        : result.map((item) => Meal.fromJson(item)).toList();
    return meals;
  }
  ...
  getFavoriteMealsById(String idMeal) async {
    ...
    return result!.isEmpty ? null : Meal.fromJson(result.first);
  }

  addFavoriteMeals(Meal meals) async {
    final db = await database;
